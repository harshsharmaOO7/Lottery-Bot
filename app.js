 ...
 * =========================================
 * Fixed: IST clock, date detection, no PDF buttons
 * Image loads from GitHub raw URL (no hotlink block)
 */
(function () {
'use strict';
...
// ── Highlight current draw tab on homepage ─────────────────────────
function highlightDrawTab() {
...
window.onImgFail = function (img) {
  ...
  if (wrap && !wrap.querySelector('.img-err')) {
    ...
    d.innerHTML = '📸 Image abhi available nahi hai<br><small>Upload hone ke baad yahan dikhegi</small>';
    wrap.appendChild(d);
  }
};

// ── Image URL builder ──────────────────────────────────────────────
// "images/nagaland-8pm-2026-04-22.jpg" → RAW_BASE + path
// "https://..." → as-is
function imgURL(rec) {
...
function findRecord(data, state, draw) {
  ...
  // 2. Latest with same draw (any date)
  for (var j = 0; j < arr.length; j++)
    if (arr[j].draw === draw)
      return { rec: arr[j], isToday: false };
  // Step 3 REMOVED: never show a different draw as fallback
...
function renderResult(rec, draw, isToday) {
  ...
  if (!isToday) showBanner(rec, draw);
    ...
    var ob = document.getElementById('old-banner');
    if (ob) ob.remove();
  }
  ...
  if (imgEl) {
    if (iu) {
      ...
      if (wrap) wrap.innerHTML =
        ...
        '<p style="font-size:.8rem;margin-top:6px;opacity:.7;">Upload hone ke baad yahan dikhegi</p>' +
        '</div>';
    }
  }
...
function renderHistory(data) {
  ...
  grid.innerHTML = items.map(function (it) {
    ...
    return '<a class="hitem" href="' + (iu || '#') + '" ' +
      (iu ? 'target="_blank" rel="noopener"' : '') + '>' +
      '<span>' + nm + ' ' + r.draw + '</span>' +
...
// ── Fetch JSON ─────────────────────────────────────────────────────
// cache:'no-store' + ?t=timestamp → browser kabhi cache use nahi karta
function loadData() {
  ...
  fetch(url, { signal: ctrl.signal, cache: 'no-store' })
    ...
    .catch(function (err) {
      ...
      // Only show error on first load fail
      var card = document.getElementById('result-card');
...
function scheduleRefresh() {
  ...
  // Near draw times → every 3 min; else every 10 min
  var nearDraw = (h === 12 && m >= 50) || (h === 13 && m <= 20) ||
                 (h === 17 && m >= 50) || (h === 18 && m <= 20) ||
                 (h === 19 && m >= 50) || (h === 20 && m <= 20);
  setTimeout(function () { loadData(); scheduleRefresh(); }, nearDraw ? 3*60*1000 : 10*60*1000);
}
